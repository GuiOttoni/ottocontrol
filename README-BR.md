...
Publicar é automático: criar uma **release no GitHub** dispara o workflow `.github/workflows/publish.yml`, que sincroniza a versão do pacote com a tag da release, builda e publica via **npm Trusted Publishing (OIDC)** — sem token/secret nenhum.

Pra buildar e testar localmente antes de criar a release:

```bash
cd package
npm run build              # gera package/dist/ (copia o server + builda o client)
npm install -g .           # opcional: testa a instalação global local
```
